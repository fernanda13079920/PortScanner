...
if 'OPENAI_API_KEY' in config:
    ...
    openai._custom_auth_header = {"Authorization": f"Bearer {config['OPENAI_API_KEY']}"}  
    ...
    if openai.api_key:
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[ERROR] Falló la petición con código {response.status_code}")
                advice = "Error al obtener respuesta del servidor de IA."
...
def generar_recomendacion(resultados, tipo_analisis):
    """
    Envía los resultados a la IA para generar una recomendación de seguridad general.
    """

    if not openai.api_key:
        return "La funcionalidad de generación de recomendaciones no está disponible."

    try:
        solicitud = (
            f"Actúa como un analista de ciberseguridad. Basado en este análisis de {tipo_analisis}: "
            f"{resultados}, analiza posibles riesgos, vulnerabilidades, amenazas conocidas, "
            f"y sugiere medidas de mitigación. Resume también el nivel de riesgo (Alto, Medio, Bajo) si es posible."
        )
        ...
        payload = {
            "model": "gpt-3.5-turbo",
            "messages": [
                {"role": "system", "content": "Eres un experto en análisis de seguridad web y redes."},
                {"role": "user", "content": solicitud}
            ],
            "temperature": 0.3,
            "max_tokens": 500
        ...
        if response.status_code == 200:
            ...
            print(f"[ERROR] Falló la petición a la IA con código {response.status_code}")
            return "Error al obtener recomendación de la IA."

    except Exception as e:
        print(f"[ERROR] Error al contactar IA: {e}")
...
def recomendacion():
    ...
    recomendacion = generar_recomendacion(resultados, tipo_analisis)

    return render_template('recomendacion.html', tipo_analisis=tipo_analisis, recomendacion=recomendacion)
...
def scan_headers(url):
    ...
    try:
        ...
        for header in ['Content-Security-Policy', 'X-Frame-Options', 'Strict-Transport-Security', 'X-Content-Type-Options', 'Referrer-Policy']:
